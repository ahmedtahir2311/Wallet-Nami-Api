...
export async function NamiWalletApi(
  NamiWalletObject: any,
  ...
  async function getRewardAddress(): Promise<string> {
    return S.RewardAddress.from_address(
      S.Address.from_bytes(Buffer.from(await getRewardAddressHex(), "hex"))
    )
      ?.to_address()
      .to_bech32();
  }
  ...
  async function _signSubmitTx(transactionRaw: Uint8Array): Promise<string> {
    ...
    const signedTx = S.Transaction.new(
      transaction.body(),
      S.TransactionWitnessSet.from_bytes(Buffer.from(witneses, "hex")),
